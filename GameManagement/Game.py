...
class Game:
    ...
    def __init__(self, game_token, parsed_data, p_one_ip, p_one_port, socket, listener, db):
        ...
        self.player_one: Player = Player(game_token=self.game_token,
                                         ...
                                         avatar=self.db.getAvatar(parsed_data["username"]))
    ...
    def add_player_two(self, username, signonToken, pTwoIp, pTwoPort, socket):
        ...
        self.player_two = Player(game_token=self.game_token,
                                 ...
                                 avatar=self.db.getAvatar(username))
    ...
    def forward_match_communication(self, requester_username, communication_obj):
        if (requester_username == self.player_one.username):
            self.player_two.socket.send(str(communication_obj).encode("utf-8"))
        # self.player_one.socket.send(str(succsess_response()).encode("utf-8"))
        elif (requester_username == self.player_two.username):
        ...
        # self.player_two.socket.send(str(succsess_response()).encode("utf-8"))
            ...
            print(f"{requester_username} was not found in this game")
    ...
    def create_random_game_response(self):
        ...
        response["player_one"] = self.player_one.username
